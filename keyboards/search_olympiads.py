...
def olymp_select_markup(olympiads: list):
    ...
    markup.row_width = 1
    markup.add(*(InlineKeyboardButton(olymp[0], callback_data=olymp[0]) for olymp in olympiads))
    return markup
...
def confirm_select_olymp_markup(bonuses):
    ...
    markup = InlineKeyboardMarkup()
    markup.row_width = 2
    markup.add(*[InlineKeyboardButton(degrs[index], callback_data=degrs[index])
                 for index, bonus in enumerate(bonuses) if bonus],
...
def calculate_or_add_markup():
    markup = InlineKeyboardMarkup()
    markup.row_width = 2
    markup.add(InlineKeyboardButton('Добавить олимпиаду', callback_data='add_olymp'),
...
def next_move(call: telebot.types.CallbackQuery):
    if call.data == 'add_olymp':
        ...
        with bot.retrieve_data(call.from_user.id, message.chat.id) as data:
            bot.edit_message_text(f'Выбран университет: {data["selected_university"]}\nНапишите название олимпиады:',
        ...
        with bot.retrieve_data(call.from_user.id, message.chat.id) as data:
            ...
            bot.edit_message_text('Выбран университет: {university}\n'
...
def confirm(call: telebot.types.CallbackQuery):
    ...
    with bot.retrieve_data(call.from_user.id, message.chat.id) as data:
        ...
        bot.edit_message_text(text=f'Выбран университет: {data["selected_university"]}\nВыбрана олимпиада "{call.data}"\n\n'
        ...
        bot.set_state(call.from_user.id, SearchStates.confirm_adding, message.chat.id)
...
def after_confirm(call: telebot.types.CallbackQuery):
    ...
    with bot.retrieve_data(call.from_user.id, message.chat.id) as data:
        ...
        if call.data == 'back':
            ...
            bot.edit_message_text(text='Выбранный университет: {university}\n\n'
                                       'Выбранные олимпиады: \n{olymps}\n\n'

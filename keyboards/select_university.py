...
def unv_markup():
    ...
    markup.row_width = 1
    markup.add(
        *(InlineKeyboardButton(univ, callback_data=univ) for univ in universities)
    )
    return markup


@bot.callback_query_handler(
    func=lambda call: True, state=SearchStates.select_universities
)
def university_callback_query(call: telebot.types.CallbackQuery):
    ...
    bot.edit_message_reply_markup(
        message.chat.id, message.message_id, reply_markup=calculate_or_add_markup()
    )
